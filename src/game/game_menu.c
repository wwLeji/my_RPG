...
{
    sfSprite_setPosition(menu->settings_sprite[7], (sfVector2f){1300, 920});
    sfRenderWindow_drawSprite(window->window, menu->settings_sprite[7], NULL);
...
void game_menu(window_t *window, menu_t *menu, player_t *player, map_t *map)
{
    if (window->is_in_game_menu == 1) {
        ...
        button_case_game_menu(window, menu, map, player);
