...
static void draw_button(window_t *window, button_t *button)
{
    ...
    && window->is_in_game == 0 && window->is_in_load_new == 0) {
    ...
    if (window->is_in_load_new == 1) {
        sfRenderWindow_drawSprite(window->window,
        button->sprite_load_save[0], NULL);
        sfRenderWindow_drawSprite(window->window,
        button->sprite_load_save[1], NULL);
    }
}

void button_case_menu(window_t *window, menu_t *menu, player_t *player,
map_t *map)
{
    ...
    buttons_new_load(window, menu->button, player, map);
