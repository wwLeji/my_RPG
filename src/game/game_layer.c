...
void draw_end(player_t *player, window_t *window, menu_t *menu)
{
    sfRenderWindow_drawSprite(window->window, menu->end[0], NULL);
    sfRenderWindow_drawSprite(window->window, menu->end[1], NULL);
}
...
map_t *map, player_t *player)
{
    if (window->is_in_menu == 1) {
        draw_menu(menu, window->window, menu->sprite_title, player);
        set_pos_paralax(menu);
        move_paralax(menu);
        if (player->end_game == 1)
            draw_end(player, window, menu);
...
map_t *map, player_t *player)
{
    if (window->is_in_menu == 1) {
        ...
        draw_bubble(player, window);
        draw_talk(menu, player, window);
    } if (window->menu_anim_fade == 1 || (player->end_game == 1)) {
        sfRenderWindow_drawRectangleShape(window->window, menu->fade, NULL);
    } if (player->end_game == 1) {
        sfRenderWindow_drawSprite(window->window, menu->end[2], NULL);
        sfRenderWindow_drawSprite(window->window, menu->end[3], NULL);
    }
}
