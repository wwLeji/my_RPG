...
player_t *player, map_t *map)
{
    ...
    && pos_mouse.x > 650 && pos_mouse.x < 925)
    && pos_mouse.y > 450 && pos_mouse.y < 522) {
        if (sfMouse_isButtonPressed(sfMouseLeft) == true) {
            sfSprite_setTextureRect(button->sprite_load_save[0],
            pos_rect_second);
            window->is_in_load_new = 0; window->menu_anim_1 = 1;
        } else
            sfSprite_setTextureRect(button->sprite_load_save[0],
            pos_rect_third);
    } else
        sfSprite_setTextureRect(button->sprite_load_save[0],
        pos_rect_origin);
    buttons_new_load2(window, button, player, map);
}
...
sfVector2i pos_mouse)
{
    ...
    if ((window->menu_anim_1 == 0 && window->menu_anim_2 == 0
    && window->is_in_game == 0 && window->is_in_settings == 0
    && window->is_in_how_play == 0 && window->is_in_load_new == 0
